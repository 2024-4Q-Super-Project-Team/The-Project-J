...
float4 main(STD_VS_OUTPUT input) : SV_TARGET
{
    ...
    // 감마 디코딩
    if (USE_MAP(DIFFUSE_MAP) >= TRUE && HAS_MAP(MapType) >= TRUE)
    {
        MapColor[DIFFUSE_MAP].rgb = pow(MapColor[DIFFUSE_MAP].rgb, GAMMA);
    }
    if (USE_MAP(EMISSIVE_MAP) >= TRUE && HAS_MAP(MapType) >= TRUE)
    {
    ...
    if (USE_MAP(NORMAL_MAP) >= TRUE && HAS_MAP(MapType) >= TRUE)
    {
    ...
    float3 L = float3(0.f, 0.f, 0.f); // 빛 벡터
