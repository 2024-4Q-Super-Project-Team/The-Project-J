...
GraphicsRenderTarget::GraphicsRenderTarget(GraphicsDevice* _pDevice, HWND _hWnd)
	: mGraphicsDevice(_pDevice)
	, mHwnd(_hWnd)
	, mClearColor()
{
	ID3D11Device* pDevice = _pDevice->mDevice;
	IDXGIFactory* pFactory = _pDevice->mFactory;
...
UTID GraphicsRenderTarget::GetTypeID()
{
	return _UTID_GRAPHICS_RENDERTARGET;
}

HRESULT GraphicsRenderTarget::QueryInterface(const UTID _UTID, void** _ppObject)
{
    ...
    *_ppObject = nullptr;
    switch (_UTID)
    {
    case _UTID_GRAPHICS_RENDERTARGET:
        *_ppObject = static_cast<GraphicsRenderTarget*>(this);
        break;
...
void GraphicsRenderTarget::Release()
{
...
void GraphicsRenderTarget::SetClearColor(FLOAT* _rgba)
{
