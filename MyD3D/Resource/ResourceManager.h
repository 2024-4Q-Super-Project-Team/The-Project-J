...
public:
    ...
    static std::vector<ResourceHandle>&                     GetLoadResourceList();
private:
    ...
    static std::vector<ResourceHandle> mLoadResourceList;    // 모든 핸들을 저장한게 아니라, 경로를 통해 직접 불러올 핸들만 모아둔 컨테이너 (ex. Mesh, Material은 fbx를 통해 로드하니까 여기에 포함되지 않음)
public:
